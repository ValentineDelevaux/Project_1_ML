...
def batch_iter(y, tx, batch_size, num_batches=1, shuffle=True):
    ...
    if shuffle:
        shuffle_indices = np.random.RandomState(seed=42).permutation(np.arange(data_size))
...
def calculate_logistic_loss(y, tx, w):
    ...
    loss = np.linalg.norm((-1/(tx.shape[0]))*(y.T.dot(np.log(y_predicted + epsilon))+(1-y).T.dot(np.log(1-y_predicted + epsilon))))
